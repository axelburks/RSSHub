...
module.exports = async (ctx) => {
  ...
  const url = type == 'beta' ? 'https://www.nssurge.com/mac/v4/appcast-signed-beta.xml' : 'https://www.nssurge.com/mac/v4/appcast-signed.xml';
  ...
  ctx.state.data = {
    ...
    item:
      ...
      list
        .map((index, item) => {
          let itemTitle = `${title} ${$('enclosure', item).attr('sparkle:shortVersionString')}.${$('enclosure', item).attr('sparkle:version')}`;
