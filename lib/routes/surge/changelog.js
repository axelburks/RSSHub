...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item:
            ...
            list
                .map((index, item) => {
                    const description = $('markdownDescription', item).text() ? `${$('markdownDescription', item).text().replace(/### (.+)/gi, "<b>$1</b><br>").replace(/\* (.+)/gi, "<code>$1</code><br>")}Download: ${$('enclosure', item).attr('url')}` : $('description', item).html();
                    return {
